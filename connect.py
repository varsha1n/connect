...
road=pygame.image.load((r"C:\python\background.jpg"))
button_img = pygame.image.load(r'C:\python\restart.png')


#player1
player1img=pygame.image.load(r'C:\python\ab1.png')
player1x=130
player1y=200 
player1x_change=0
player1y_change=0
...
#player2
player2img=pygame.image.load(r'C:\python\ab2.png')
player2x=600
player2y=200
player2x_change=0
player2y_change=0
...
for i in range(num_of_obstacle):
    obstacleimg.append(pygame.image.load('C:\python\pig.png'))
...
def iscollition(player1x,player1y,player2x,player2y,obstaclex,obstacley):
    ...
    distance_1o=math.sqrt((math.pow(player1x-obstaclex,2)) + (math.pow(player1y-obstacley,2)))
    distance_2o=math.sqrt((math.pow(player2x-obstaclex,2)) + (math.pow(player2y-obstacley,2)))
    if player2x-player1x-64!=0:
        y_e=(player1y+32)+(((player2y+32-player1y-32)/(player2x-player1x-64))*(obstaclex-player1x-64))
        distance_eo=math.sqrt((math.pow(y_e-obstacley,2)))
        if distance_1o<59 or distance_2o<59 or (distance_eo<5 and ((player1x<obstaclex and player2x>obstaclex) or (player1x>obstaclex and player2x<obstaclex))):
            return True
        else:
            return False
def show_score(x,y):
...
while running:
    ...
    for i in range(num_of_obstacle):
        ...
        #collision
        collision=iscollition(player1x,player1y,player2x,player2y,obstaclex[i],obstacley[i])
        if collision:
